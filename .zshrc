...
set -o vi
function cd() {
  # Get the current and target directories
  local target="$1"
  local current="$PWD"

  # If no target directory is provided, go to the home directory
  [ -z "$target" ] && target="$HOME"

  # If target is a relative path, convert it to an absolute path
  if [[ "$target" != /* ]]; then
  ...
  if [[ -d ./venv ]]; then
    ...
    if [[ -z "$VIRTUAL_ENV" ]]; then
      ...
      # Check if we moved out of the virtual environment's parent directory
      parentdir="$(dirname "$VIRTUAL_ENV")"
      if [[ "$current" != "$parentdir"* ]]; then
        deactivate
      fi
    fi
  elif [[ -n "$VIRTUAL_ENV" ]]; then
    # We moved out of a virtual environment's directory
    deactivate
  fi
}
